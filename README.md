...
function App() {
  ...
  const QuestionSection = ({ questions }) => {
    ...
    useEffect(() => {
      // Reset answers and feedback when questions change (level/course changes)
      setUserAnswers({});
      setFeedback({});
      setScore(0);
    ...
    const checkAnswers = () => {
      ...
      questions.forEach(q => {
        ...
        if (q.type === 'number') {
          // For numbers, allow a small tolerance for floating point answers
          isCorrect = Math.abs(parseFloat(userAnswer) - q.answer) < 0.001;
        } else if (q.type === 'text') {
          // Basic text comparison, case-insensitive and trim spaces
          // Also handle multiple possible answers if needed, by checking against an array of answers
          const correctAnswers = Array.isArray(q.answer) ? q.answer : [q.answer];
          isCorrect = correctAnswers.some(ans => userAnswer && userAnswer.trim().toLowerCase() === ans.toLowerCase());
        }
        // Add more types (multiple_choice, etc.) here if needed
    ...
    return (
      <div className="bg-gray-100 p-8 rounded-2xl text-gray-800 border border-gray-200 text-lg leading-relaxed mt-8">
        <h3 className="text-2xl font-bold text-gray-900 mb-6">Ejercicios - ¡Pon a prueba tus conocimientos! 📝</h3>
        <p className="mb-6">Responde las siguientes preguntas.</p>
            ...
            <li key={q.id} className="mb-4">
              ...
              <input
                ...
                className={`w-full p-3 rounded-lg border-2 focus:outline-none focus:ring-2
                  ${showResults
                    ? (feedback[q.id] === 'correct' ? 'border-green-500 focus:ring-green-300' : 'border-red-500 focus:ring-red-300')
                    : 'border-gray-300 focus:border-blue-500 focus:ring-blue-300'
                  }`}
                placeholder="Tu respuesta..."
              />
              {showResults && (
                <p className={`mt-2 text-sm font-medium ${feedback[q.id] === 'correct' ? 'text-green-600' : 'text-red-600'}`}>
                  {feedback[q.id] === 'correct' ? '¡Correcto! ✅' : `Incorrecto. La respuesta correcta es: ${Array.isArray(q.answer) ? q.answer.join(' / ') : q.answer}`}
                </p>
              )}
            </li>
          ))}
        ...
        <button
          ...
          className="mt-8 px-8 py-4 bg-purple-600 text-white rounded-full shadow-lg hover:bg-purple-700 transition-colors duration-300 text-xl font-bold block mx-auto"
        >
          Verificar Respuestas ✨
        </button>
        {showResults && (
          <div className="mt-8 text-center text-2xl font-bold">
            <p>Tu puntuación: <span className="text-blue-700">{score}</span> de <span className="text-blue-700">{questions.length}</span></p>
            {score === questions.length && <p className="text-green-600 mt-2">¡Excelente trabajo! 🎉</p>}
            {score < questions.length && <p className="text-orange-600 mt-2">¡Sigue practicando para mejorar! 💪</p>}
          </div>
        )}
      </div>
    );
  ...
  // Moved CourseCard and CourseDetail components inside App to resolve ReferenceError
  const CourseCard = ({ course, onSelect }) => (
    <div
      className="bg-white p-8 rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer transform hover:-translate-y-2 flex flex-col items-center text-center border border-gray-100"
      onClick={() => {
        setSelectedCourse(course);
        setSelectedLevel('basico'); // Reset level to basic when a new course is selected
      }}
    >
      <div className="text-6xl mb-5"> {/* Increased icon size */}
        {course.id === 'matematicas' && '➕'}
        {course.id === 'comunicacion' && '🗣️'}
        {course.id === 'cyt' && '🔬'}
        {course.id === 'ingles' && '🇬🇧'}
      </div>
      <h3 className="text-3xl font-bold text-gray-900 mb-3">{course.name}</h3> {/* Bolder and larger title */}
      <p className="text-gray-600 text-base leading-relaxed">{course.description}</p> {/* Better line height */}
    </div>
  );
  ...
  const CourseDetail = ({ course, onBack }) => {
    ...
    return (
      <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-4xl border border-gray-100">
        <button
          onClick={onBack}
          className="mb-8 px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-full shadow-lg hover:shadow-xl hover:from-blue-600 hover:to-indigo-700 transition-all duration-300 flex items-center text-lg font-semibold"
        >
        ...
        <h2 className="text-5xl font-extrabold text-gray-900 mb-6 leading-tight">{course.name}</h2>
        <p className="text-gray-700 text-xl mb-8 leading-relaxed">
          ¡Bienvenido al curso de {course.name}! Selecciona un nivel para comenzar tu aprendizaje.
        </p>
        ...
        <div className="bg-gray-50 p-8 rounded-2xl text-gray-800 border border-gray-200 text-lg leading-relaxed">
          <h3 className="text-2xl font-bold text-gray-900 mb-4">Teoría del Curso - Nivel {selectedLevel.charAt(0).toUpperCase() + selectedLevel.slice(1)}</h3>
          {/* Render content dynamically using dangerouslySetInnerHTML */}
          <div dangerouslySetInnerHTML={{ __html: currentLevelContent.theory }} />
        </div>

        {currentLevelContent.questions && currentLevelContent.questions.length > 0 && (
          <QuestionSection questions={currentLevelContent.questions} />
        )}
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-blue-100 font-sans flex flex-col items-center justify-center p-6">
      <style>
        {`
        body {
      ...
      <div className="w-full max-w-7xl mx-auto py-10">
        <h1 className="text-6xl font-extrabold text-center text-gray-900 mb-16 drop-shadow-lg leading-tight">
          <span className="text-red-600">Chisfuerzo:</span> <span className="text-blue-600">Aprende</span> y <span className="text-purple-600">Crece</span>
        </h1>
